...
import { useTheme, LightTheme } from '../hooks/use-theme';
...
// New helper: soft RGBA background from hex
    ...
    return (
      <View style={{ height, justifyContent: 'center' }} {...pan.panHandlers}>
        ...
        <View style={{ position: 'absolute', left: `${(hue / 360) * 100}%`, transform: [{ translateX: -knobRadius }] }}>
          <View style={{
            width: knobRadius * 2, height: knobRadius * 2, borderRadius: knobRadius,
...
const SettingsScreen: React.FC = () => {
  ...
  // store last meaningful theme key locally to avoid unnecessary updates
  const lastKeyRef = useRef<string>(
    `${String(theme.dark)}|${String((theme.colors as any).primary ?? '')}|${String((theme.colors as any).background ?? '')}`
  );
  ...
  useEffect(() => {
    ...
    // update lastKeyRef when theme actually changes from provider
    lastKeyRef.current = `${String(theme.dark)}|${String((theme.colors as any).primary ?? '')}|${String((theme.colors as any).background ?? '')}`;
  }, [theme]);

  // Build a theme from selected accent. Uses current theme.dark to pick base;
  // background is a softened rgba of the accent.
  const buildThemeFromAccent = useCallback((accent: string) => {
  ...
  // shallow key builder (same meaningful fields the provider checks)
  const makeKey = (t: any) => `${String(t.dark)}|${String((t.colors as any).primary ?? '')}|${String((t.colors as any).background ?? '')}`;

  const saveTheme = async () => {
    try {
      const newTheme = buildThemeFromAccent(accentHex);
      const currentKey = lastKeyRef.current;
      const newKey = makeKey(newTheme);
      ...
      // update ref immediately — prevents flash if provider rehydrates slowly
      lastKeyRef.current = newKey;

      // call provider to persist + set theme
